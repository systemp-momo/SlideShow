//
//  SPSmbAppSettingsTests.m
//  SPSmbSlideshow
//
//  Created by systemp on 2016/06/26.
//  Copyright © 2016年 systemp. All rights reserved.
//

#import <XCTest/XCTest.h>
#import "SPSmbAppSettings.h"

@interface SPSmbAppSettingsTests : XCTestCase
@property (strong)SPSmbAppSettings* appSettings;
@end

@implementation SPSmbAppSettingsTests

- (void)setUp {
    [super setUp];
    // Put setup code here. This method is called before the invocation of each test method in the class.
}

- (void)tearDown {
    // Put teardown code here. This method is called after the invocation of each test method in the class.
    [super tearDown];
}

- (void)testExample {
    // This is an example of a functional test case.
    // Use XCTAssert and related functions to verify your tests produce the correct results.
    self.appSettings = [[SPSmbAppSettings alloc] init];
    
    NSString *key = [SPSmbAppSettings exludedArrayItemKey];
    XCTAssertEqualObjects(key, @"Excluded");
    
    self.appSettings = nil;
}

- (void)testPerformanceExample {
    // This is an example of a performance test case.
    [self measureBlock:^{
        // Put the code you want to measure the time of here.
    }];
}

@end
